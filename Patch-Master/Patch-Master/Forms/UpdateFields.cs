...
{
    public partial class UpdateFields : Form
    {
        public static string UPDATESTRING = string.Empty;
        public static string firstTable;
        public static string selectedDatbase = Requirements.SELECTEDDATABSENAME;
      ...
      {
           firstTable = UpdateQueryBuilder.firstSelectedTable;
            CmbTable_1.Text = firstTable;
            ColumnList = LoadAllColumns(selectedDatbase, firstTable);
            if (ColumnList != null && ColumnList.Count() > 0 && CmbColumn_1 != null)
            {
                CmbColumn_1.Items.Clear();
                foreach (string item in ColumnList)
                {
                    CmbColumn_1.Items.Add(item);
                }
            }

            //List<string> tableList = UpdateQueryBuilder.UpdateTableList;
       ...
       //    ComboBox TableSelectComboBox = sender as ComboBox;
        ...
        private void butnAddNewRow_1_Click(object sender, EventArgs e)
        {
            ...
            int newGroupOrderNumber = GroupController.GroupBoxOrderNumber + 1;

            //ReorderWhenAddingTCRow(newGroupOrderNumber);

            ReorderWhenAddingConditionRow(newGroupOrderNumber);
            conditionRowGenerator(",", newGroupOrderNumber);
        ...
        private void btnConAND_Click(object sender, EventArgs e)
        {
            ...
            Button conditionAddingButtton1 = sender as Button;

            if (conditionAddingButtton1 == null) // just to be on the safe side
                return;

            string Controllname = conditionAddingButtton1.Parent.Name.ToString();
        ...
        private void conditionRowGenerator(String OperatorType, int newGroupOrderNumber)
        {
          ...
          //  }
            ...
            ComboBox CmbColumn = new ComboBox();
            {
                ...
                if (ColumnList != null && ColumnList.Count() > 0 && CmbColumn != null)
                {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            UpdateValueFields tableColumnSelector = new UpdateValueFields(AddTableColumnBtn.Name.ToString(), this);
                    ...
                    if (ctn is TextBox)
                    {
                        ...
                        if (ControllerName != null && ControllerName.Split("_").Length > 0 && ctn.Visible ==true)
                        {
                            ...
                            if (ControllerName.Split("_")[0] == "textValue")
                            {
                                ...
                                var match = Value2.IndexOfAny(new char[] { '+', '-', '*', '/','.' });
                                if (match == -1)
                                {
                                    Value2 = "'" + Value2 + "'";
                                }
